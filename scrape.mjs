...
async function scrapeResultsUrl() {
  const response = await fetch(
    "https://www.cdcr.ca.gov/bph/parole-suitability-hearing-results/"
  );
...
console.log(
  `🥸 Found ${results.length.toLocaleString("en-US")} hearing results`
);
const merged = merge(existing, results, (d) => {
  return `${d.cdc}-${d["scheduled-date"]}`;
});
console.log(
  `💾 Saving all ${merged.length.toLocaleString("en-US")} hearing results`
);
